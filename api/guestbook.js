// 방명록 시스템 API (Vercel Serverless Function)
// Google Apps Script의 방명록 관련 기능만 프록시
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (!action || !allowedActions.includes(action)) {
                return res.status(400).json({
                    success: false,
                    error: 'Invalid or missing action for guestbook API',
        ...
        console.log(`🔄 Proxying ${req.method} request to Google Apps Script for guestbook`);
